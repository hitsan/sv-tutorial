...
### 1. レジスタ (register/)
- **概要**: データを保持し、クロック信号に同期してデータを更新する基本的な記憶素子
- **学習内容**:
...
- **重要ポイント**:
  ...
  - `posedge`/`negedge`の理解
  - ノンブロッキング代入(`<=`)の使用
- **ファイル**: `register/`（8種類の実装例）

### 2. 順序回路の合成例 (seq_examples/)
- **概要**: 動作記述から特定の回路構造への合成パターンを学ぶための実例集
- **学習内容**:
  - 動作記述から特定の構造への合成
  - よくある間違いとその修正方法
...
### 3. カウンタ (counter/)
- **概要**: クロックごとに値を増減させ、特定の範囲で循環するカウンタ回路
- **学習内容**:
  - アップカウンタ、ダウンカウンタ
  - ロード可能なカウンタ
  - 任意の初期値と上限値
  - イネーブル制御
- **ファイル**: `counter/`（10種類の実装例：BCD、グレイコード、ワンホット等）

### 4. シフトレジスタ (shift_register/)
- **概要**: データを左または右に1ビットずつシフトさせるレジスタ回路
- **学習内容**:
  - 左シフト、右シフト
  - パラレルロード
  - シリアル入力/出力
...
### 5. パイプライン化された乗算器 (multiplier_pipelined/)
- **概要**: 乗算処理を複数のステージに分割し、動作周波数とスループットを向上させたパイプライン回路
- **学習内容**:
  - パイプライン化の基本概念
  - レイテンシとスループットの違い
  - 組み合わせ回路から順序回路への変換
...
- **重要ポイント**:
  ...
  - パイプラインステージの設計
- **ファイル**: `multiplier_pipelined/`

### 6. パイプライン化された乗算器の各種実装 (multiplier_pipelined/)
- **概要**: 様々なパイプライン化アーキテクチャによる乗算器の実装バリエーション
- **学習内容**:
...
- **実装ファイル**:
  - `multiplier_pipelined_multistage.sv`: 3ステージパイプライン実装
  - `multiplier_pipelined_array.sv`: シストリックアレイ実装
