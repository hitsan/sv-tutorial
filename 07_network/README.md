...
- **概要**: UDPパケットの受信パーサモジュール（簡易版、IPv4前提）
- **学習内容**:
  - UDPヘッダ解析
  - ビッグエンディアン処理
  - ペイロード抽出
...
- **仕様（学習向け提案）**:
  - 入力は8-bitの`data_in`と`valid_in`（UDPヘッダから開始）。
  - UDPヘッダ：Source Port(2) → Dest Port(2) → Length(2) → Checksum(2)の順でパース（ビッグエンディアン）。
  - ヘッダ解析完了時に`src_port`と`dst_port`を出力し、パケット終了まで保持。
  - **Lengthフィールドの処理**：
    - Length値からヘッダ（8バイト）を引いた分だけペイロードを出力。
    - 実際のデータがLengthより多く来ても、Length分のペイロードのみ出力（残りは破棄）。
    - 実際のデータがLengthより少ない場合（`valid_in=0`で途切れた）、その時点で終了。
  - パケット終端は以下のいずれかで判定：
    - Lengthフィールドで指定された分のペイロードを出力完了。
    - または、`valid_in`の低下。
  - ペイロード転送中の`valid_in=0`は一時停止として扱い、再度`valid_in=1`で継続（バースト転送対応）。
  - ヘッダ受信中（8バイト未満）に`valid_in=0`が発生した場合はエラーとしてIDLEに戻る。
  - チェックサム検証は省略（簡易版）。
  - ポート番号フィルタリングは実装しない（上位層の責務）。
