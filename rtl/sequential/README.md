...
### 6. 📝 パイプライン化された乗算器の各種実装 (exercises/multiplier_pipelined_variants.sv)
- **学習内容**:
  - 様々なパイプライン化アーキテクチャの比較
  - Boothエンコーディング (Radix-2)
  - Wallace Tree リダクション
...
- **5つの実装バリアント**:
  1. **Multi-Stage Pipeline (3-4段)**: 基本的な多段パイプライン
     - 2ステージを3-4ステージに拡張
     - ステージ数増加によるクリティカルパス短縮
     - レイテンシ: 3-4 cycles

  2. **Booth Encoding (Radix-2)**: アルゴリズムによる部分積削減
     - Booth符号化で部分積を削減（8個→4-5個）
     - 符号付き演算の自然な処理
     - レイテンシ: 3 cycles
...
|---------|----------|----------|-------|-------|---------|
| Multi-Stage | 3-4 | 3-4 | 低 | 高 | パイプライン基礎 |
| Booth | 3 | 3 | 中 | 高 | アルゴリズム最適化 |
| Wallace | 4 | 4 | 高 | 低 | 並列削減の極限 |
| Array | 8 | 8 | 中 | 最高 | シストリックアレイ |
| CSA | 4 | 4 | 高 | 中 | 規則的並列削減 |

- **重要な学習ポイント**:
  - **アーキテクチャ選択**: 用途に応じた最適な実装の選択
    - 高速化優先 → Wallace Tree
    - 規則性優先 → Array Multiplier
    - バランス型 → Multi-Stage, Booth
  - **トレードオフ**: レイテンシ vs 複雑度 vs 規則性
  - **並列化の手法**: 空間並列（Array）vs 時間並列（Pipeline）
  - **アルゴリズムの影響**: Boothの部分積削減、CSAのキャリー遅延

- **各アルゴリズムの詳細**:

  **Booth Encoding (Radix-2)**:
  - 乗数のビットパターンを効率的に処理
  - 連続した1のビット列を減算で処理: 0111 = 1000 - 0001
  - 符号付き乗算が自然に実装できる
  - ハードウェア量削減の古典的手法

  **Wallace Tree**:
  - 部分積を並列に加算するツリー構造
  - 3:2圧縮器(Full Adder)で段階的に削減
  - O(log N)の深さで高速
  - 配線が不規則だが最も高速
  ...
  **Carry-Save Adder (CSA)**:
  - キャリー伝搬を最終段まで遅延
  - 中間結果を(sum, carry)の2ベクトルで保持
  - 各段の遅延時間が均一
  - Wallace Treeより規則的な構造

- **演習**: 5つの実装を完成させ、動作を比較してください
  - 各アルゴリズムの特徴を理解する
  - レイテンシの違いを確認する
  - 全バリアントが同じ結果を出力することを検証する

- **テストベンチ**: `multiplier_pipelined_variants_tb.sv`
  - 5つの実装を統合的にテスト
  - 基本・境界値・パターン・ランダムテスト
  - クロスチェック: 全実装の結果一致を確認
